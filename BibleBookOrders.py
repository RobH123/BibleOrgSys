...
#   Last modified: 2010-11-10 (also update versionString below)
#
...
class BibleBookOrdersConvertor:
    ...
    def importDataToPython( self ):
        ...
        for bookOrderSystemCode in self.systems.keys():
            ...
            for bookElement in self.systems[bookOrderSystemCode]["tree"]:
                ...
                bookDataDict[bookRA] = ID
                assert( ID not in idDataDict )
                idDataDict[ID] = bookRA
    ...
    def exportDataToPython( self, filepath=None ):
        ...
        assert( self.systems )
        if not filepath: filepath = os.path.join( "DerivedFiles", BibleBookOrdersConvertor.filenameBase + ".py" )
        ...
        with open( filepath, 'wt' ) as myFile:
            ...
            for systemName in bookOrderSystemDict:
                myFile.write( "#\n# %s\n" % ( systemName ) )
                exportPythonDict( myFile, bookOrderSystemDict[systemName][0], "chapterVerseDict", "id", "referenceAbbreviation, SBLAbbreviation, OSISAbbreviation, ParatextAbbreviation, ParatextNumberString, nameEnglish (comment only)" )
                exportPythonDict( myFile, bookOrderSystemDict[systemName][1], "omittedVersesDict", "id", "referenceAbbreviation, SBLAbbreviation, OSISAbbreviation, ParatextAbbreviation, ParatextNumberString, nameEnglish (comment only)" )
    ...
    def exportDataToC( self, filepath=None ):
        ...
        assert( self.systems )
        if not filepath: filepath = os.path.join( "DerivedFiles", BibleBookOrdersConvertor.filenameBase + ".h" )
        print( "Exporting to %s..." % ( filepath ) )
